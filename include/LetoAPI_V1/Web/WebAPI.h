...
#ifndef INC_LETO_API_V1_WEB_WEB_API_H_
#define INC_LETO_API_V1_WEB_WEB_API_H_
...
struct WebAPI_V1
{
	 ...
	 */
	bool (*const CreateConnection) (struct WebConnection_V1* connection, uint8_t channel, uint8_t port, uint32_t id, WebConnection_V1_Callback callback);
	 ...
	 */
	void (*const CloseConnection) (struct WebConnection_V1 connection);
	 ...
	 */
	void (*const SendData) (struct WebConnection_V1 connection, const void* data, uint32_t size);
	 ...
	 */
	uint32_t (*const GetDevicesNear) (struct WebDeviceInfo_V1* info, uint32_t available);
